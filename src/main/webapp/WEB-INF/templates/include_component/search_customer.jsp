...
<select class="form-control multiple-select hidden" multiple="multiple" id="customerIds"
        name="customerIds">
    <c:choose>
            ...
            <c:choose>
                <c:when test="${customers[0]['class'].name eq uaaCustomerBeanClassName}">
                    <c:forEach var="item" items="${customers}">
                        <option value="${item.id}">${item.displayName}&nbsp;(${item.msisdn})</option>
                    </c:forEach>
                </c:when>
                <c:otherwise>
                    <c:forEach var="item" items="${customers}">
                        <option value="${item.id}">${item.fullName}&nbsp;(${item.msisdn})</option>
                    </c:forEach>
                </c:otherwise>
            </c:choose>
        </c:when>
