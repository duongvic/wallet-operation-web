    ...
    <section role="main" class="content-body">
      ...
      <div class="content-body-wrap">
        <div class="container-fluid">
          <div class="h4 mb-md">
            <spring:message code="service.exportcard.confirm.subnavigate.title"/>
          </div>
        </div>
            ...
            <div id="tab1" class="tab-pane active">
              <section class="panel">
                <div class="panel-body report_search_form pb-none">
                  <div class="row">
                    <form name="telco" id="telco" action="${epinConURI}/epin-export" method="post" >
                              ...
                              <table class="table table-bordered table-striped mb-none">
                                ...
                                <c:forEach var="item" items="${listPOdetail }" varStatus="status">
                                  <tr class="${item.cardType }">
                                    <input type="hidden" name="telco_${status.index +1}" value="${item.cardType }"/>
                                    <input type="hidden" name="value_${status.index +1}" value="${item.faceValue }"/>
                                    <input type="hidden" name="quantity_${status.index +1}" value="${item.quantity }"/>
                                    <input type="hidden" name="status_${status.index +1}" value="${item.status }"/>
                        ...
                        <div class="form-group pb-none">
                          <c:choose>
                            <c:when test="${perExportEpin}">
                              ...
                              <button type="submit" value="export" class="btn btn-success pull-right ml-xs" ${disabledNext} style="${disabledNext eq 'disabled' ? 'opacity: 0.5;' : ''}"
                                      data-loading-text="<i class='fa fa-spinner fa-spin '></i> ${waiting}"><spring:message code="common.btn.export"/>&nbsp;<i class="fa fa-arrow-right"></i></button>

                              <button type="submit" value="reject" class="btn btn-danger pull-right"
                                      data-loading-text="<i class='fa fa-spinner fa-spin '></i> ${waiting}"><i class="fa fa-times-circle"></i>&nbsp;<spring:message code="common.btn.reject"/></button>

                              <button type="button" value="previous" id="stepprevious" class="btn btn-default"><i class="fa fa-arrow-left"></i>&nbsp;<spring:message code="common.btn.back"/></button>
                            </c:when>
                            <c:otherwise>
                              <input type="hidden" name="action" value="next"/>
                              <input type="hidden" name="pageRequest" value="${pageRequest }"/>
                              ...
                              <button type="button" value="previous" id="stepprevious" class="fr mr-sm submit_bt nomal_color_bk"><spring:message code="common.btn.back"/></button>
                            </c:otherwise>
                          </c:choose>
                          <div class="clr"></div>
                        </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    $("#stepprevious").click(function () {
      window.location.href = '${contextPath}<%=EpinPurchaseOrderController.EPIN_PO_LIST%>';
    });
  });
</script>
</body>
