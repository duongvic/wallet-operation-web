...
<head>
  <!-- Basic -->
  <meta charset="UTF-8">
  <title><spring:message code="service.exportcard.edit.title.page"/></title>
...
<section class="body">
  ...
  <c:url var="epinConUri" value="<%=EpinPurchaseOrderController.EPIN_PO_CONTROLLER%>"/>
    ...
    <section role="main" class="content-body">
          ...
          <div class="col-xs-12">
            <div class="page-header-left">
              <ol class="breadcrumbs">
                ...
                <li><span><spring:message code="service.exportcard.edit.navigate.service"/></span></li>
                <li><span>
                <c:choose>
                  <c:when test="${perExportEpin eq true}">
                    <a href="<c:url value="${urlEpinPoList}"/>" id="hight-title" class="hight-title"><spring:message code="service.exportcard.edit.navigate.exportcard"/></a>
                  </c:when>
                  <c:otherwise>
                    <a href="<c:url value="${urlEpinPoList}?search=&range=&search=Search&status=1&status=2&multiselect=1&multiselect=2"/>" id="hight-title" class="hight-title"> <spring:message code="service.exportcard.edit.navigate.exportcard"/> </a>
                  </c:otherwise>
                </c:choose>
                </span></li>
                <li><span class="nav-active"><spring:message code="service.exportcard.edit.navigate.editmpo"/></span></li>
              </ol>
            </div>
          </div>
        </div>
      ...
      <div class="content-body-wrap">
        <div class="container-fluid">
          ...
          <div class="h4 mb-md">
            <spring:message code="service.exportcard.edit.subnavigate.title"/>
          </div>
              ...
              <section class="panel">
                <div class="panel-body report_search_form pb-none">
                  <div class="row">
                    <form id="tblInputTel">
                      <div class="tab-content">
                        <div class="form-group">
                          <label class="col-md-1 control-label" for="note">*<spring:message code="service.exportcard.create.note"/></label>
                        ...
                        <div class="panel-body report_search_form pb-none">
                          ...
                          <div class="col-xs-12 report_search_bt pb-none pt-md">
                            <a href="#" class="fr add-tel-po">
                              <i class="fa fa-plus"></i>&nbsp;<spring:message code="service.exportcard.edit.card.add"/>
                            </a>
                          </div>
                        </div>
                      </div>
                    ...
                    <sec:authorize access="hasAnyRole('MERCHANT','CUSTOMER')" var="perEditNext">
                    <c:url var="epinConUri" value="<%=EpinPurchaseOrderController.EPIN_PO_CONTROLLER%>"/>
                    <form name="telco" action="${epinConUri}/edit-po" method="post">
                      </sec:authorize>

                      <input type="hidden" name="totalTelco" value="${purchaseOrderDetails.size()}"/>
                      <input type="hidden" name="poCode" value="${purchaseOrder.poCode}"/>
                      <input type="hidden" name="poMerchantId" value="${param.id != null ? param.id : poMerchantId }"/>
                      <input type="hidden" name="totalQuantity" value="${purchaseOrder.totalQuantity}"/>

                      <section class="panel panel-default po-detail-line">
                        <div class="pl-xs" style="font-size: 13px;">
                          <spring:message code="service.exportcard.edit.table.header.totalmoney"/>&nbsp; <b class="b-money primary_color">${purchaseOrder.totalValue}</b> <b class="primary_color">VND</b>&nbsp;|&nbsp;
                          <spring:message code="service.exportcard.edit.table.header.quantity"/>&nbsp; <b class="b-quantity primary_color">${purchaseOrder.totalQuantity}</b>
                        </div>

                        <spring:message code="service.exportcard.edit.card.available" var="langAvailable"/>
                        <div class="panel-body">
                          <div class="table-responsive">
                            <table class="table table-bordered table-striped mb-none" id="tableMPOdetail">
                              <thead>
                              <tr>
                                <th><spring:message code="service.exportcard.edit.table.column.no"/></th>
                                <th style="width:10%"><spring:message code="service.exportcard.edit.table.column.cardtype"/></th>
                                <th class="text-right"><spring:message code="service.exportcard.edit.table.column.facevalue"/></th>
                                <th class="text-right"><spring:message code="service.exportcard.edit.table.column.quantity"/></th>
                                <th class="text-center"><spring:message code="service.exportcard.edit.table.column.status"/></th>
                                <th class="text-center"><spring:message code="service.exportcard.edit.table.column.action"/></th>
                              </tr>
                              </thead>
                              <tbody>
                      ...
                      <div class="row content-body-wrap">
                        <div class="col-md-12 col-lg-12 col-xl-12">
                          <div class="panel-body edit_profile">
                            ...
                            <c:choose>
                              <c:when test="${perEditNext eq true}">
                                <button type="submit" class="mt-md btn btn-success fr po-next-step" id="btnNextStep"
                                        data-loading-text="<i class='fa fa-spinner fa-spin '></i> ${waiting}"><spring:message code="button.next"/></button>
                              </c:when>
                              <c:otherwise>
                                <button type="button" onclick="disagree()" class="mt-md btn btn-success fr"><spring:message code="button.next"/></button>
                              </c:otherwise>
                            </c:choose>
                          </div>
                        </div>
    ...
    $('.add-tel-po').click(function () {
      if ($('#tblInputTel').valid()) {
        if ($('.telco input[name=telco]:checked').length > 0) {
          ...
          $('.inputtelco').each(function (index, value) {
            ...
            if (amount != '0' && amount != '') {
              ...
              poLine += '<td class="stt">' + i + '</td>' +
                  '<td>' + telco.toLocaleString('en-US') + '<input type="hidden" name="telco_' + i + '" value="' + telco + '" /></td>' +
          ...
          if (totalQuantity + curQuantity > 10000) {
            $.MessageBox({message: '<spring:message code="popup.message.warning.card.limit"/>'});
            return false;
          } else {
