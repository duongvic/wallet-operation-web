<%@ page import="static vn.mog.ewallet.operation.web.controller.fundin.FundInController.FUND_IN_HISTORY_CONTROLLER" %>
<%@ include file="../include_page/taglibs.jsp" %>

<spring:message code="fundin.search.placeholder" var="textPlaceHolder" scope="request"/>
<spring:message code="fundin.search.btn.search" var="btnSearch" scope="request"/>
...
<c:url var="fundInControlUri" value="<%=FUND_IN_HISTORY_CONTROLLER%>"/>
...
<section class="panel search_payment panel-default">
  <div class="panel-body pt-none">
    <form action="#" method="GET" id="tbl-filter"  class="mb-md">
      ...
      <div class="form-group ml-none mr-none">
        <div class="input-group input-group-icon">
          ...
          <input type="text" id="search" name="search" class="form-control" placeholder="${textPlaceHolder}" value="${param.search }"/>
        </div>
      </div>

      <c:if test="${!perActionNotGetAllAccount}">
        <select class="js-data-example-ajax-account-fundin" multiple="multiple" name="customerIds">
        </select>
      </c:if>
      ...
      <div class="form-inline" style="margin-top: 9px">
        ...
        <div class="pull-right form-responsive bt-plt">
          ...
          <%--<sec:authorize access="hasAnyRole('ADMIN_OPERATION','STAFF','SALESUPPORT_LEADER','SALESUPPORT_LEADER','SALESUPPORT','FINANCE')" var="permitSearchCustomer">--%>
          ...
          <jsp:include page="../include_component/search_payment_channel_save_params.jsp"/>
    ...
    <div>
      <spring:message code="fundin.table.header.transaction"/>&nbsp;
      <span class="primary_color text-semibold">${ewallet:formatNumber(total)}</span>&nbsp;|&nbsp;
      <spring:message code="fundin.table.header.amount"/>&nbsp;
    ...
    <spring:message code="fundin.table.column.no" var="colNo"/>
    <spring:message code="fundin.table.column.tranid" var="colTranId"/>
    <spring:message code="transaction.api.table.request_id" var="colRId"/>
    <spring:message code="fundin.table.column.merchant" var="colMerchant"/>
    <spring:message var="colSourceOfFund" code="transaction.api.table.source.of.fund"/>
    <spring:message code="fundin.table.column.createdTime" var="colTime"/>
    <spring:message code="fundin.table.column.channel" var="colService"/>
    <spring:message code="fundin.table.column.note" var="colNote"/>
    <spring:message code="fundin.table.column.amount" var="colAmount"/>
    ...
    <spring:message code="fundin.table.column.prebalance" var="colPreBalance"/>
    <spring:message code="fundin.table.column.postbalance" var="colPostBalance"/>
    <spring:message code="fundin.table.column.status" var="colStatus"/>
    <spring:message code="fundin.table.column.action" var="colAction"/>
    ...
    <display:table name="list" id="item"
      ...
      <display:column title="${colMerchant}" property="payeeUsername"/>

      <display:column title="${colSourceOfFund}" property="sourceOfFundId"/>
      <display:column title="${colService}">
        <spring:message code="${item.textOrderChannel()}"/>
      </display:column>
      <display:column title="CHANNEL">
        <spring:message code="${item.paymentChannelId}"/>
      </display:column>

      <display:column title="${colNote}">
        <spring:message code="${item.orderInfo}"/>
      </display:column>

      <display:column title="${colAmount}" class="col-number-header" headerClass="col-number-header">${ewallet:formatNumber(item.amount)}</display:column>
      ...
      <display:column title="${colPreBalance}" class="col-number-header" headerClass="col-number-header">${ewallet:formatNumber(item.preBalance)}</display:column>
      <display:column title="${colPostBalance}" class="col-number-header" headerClass="col-number-header">${ewallet:formatNumber(item.postBalance)}</display:column>
...
<script>
  $('.js-data-example-ajax-account-fundin').select2({
    width: "100%",
