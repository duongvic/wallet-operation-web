...
<display:table name="transactions" id="item"
  ...
  <c:set var="backgroundClass" value="${TxnStatus.FAILED eq item.transactionStatus ? 'levelTwoWarning' : ''}"/>
  ...
  <display:column title="${colAction}" class="action_icon center ${backgroundClass}"
    ...
    <sec:authorize access="hasAnyRole('ADMIN_OPERATION','CUSTOMERCARE_MANAGER','CUSTOMERCARE')" var="perReversal"/>
    <c:if test="${( item.transactionStatus eq 10 || item.transactionStatus eq 10 )&& perReversal}">
  ...
  <display:column title="${colTransID}" class="${backgroundClass}">
    <a class="app-name detail-link link-active" href="#"
  ...
  <display:column title="${colMerchant}" class="${backgroundClass}"
  ...
  <display:column title="${colProvider}" class="${backgroundClass}" property="providerCode"/>
  ...
  <display:column title="${colAction}">
    <%--<a href="${contextPath}<%=ReconciliationController.RECON_DETAIL%>?quickSearch=${item.providerCode}"--%>
       <%--title="<spring:message code="common.title.detail" />">--%>
      <%--<i class="fa fa-info-circle" aria-hidden="true"></i>--%>
    <%--</a>--%>
    <%--<a href="${contextPath}<%=ReconciliationController.RECON_DETAIL%>?provider=${item.providerCode}"--%>
