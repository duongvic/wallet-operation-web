...
<section class="panel panel-default">
  ...
  <div class="panel-body report_search_form">
    <c:choose>
      <c:when test="${tranReversal.transaction.serviceType eq 'EPIN'}">
        ...
        <c:forEach var="attribute" items="${tranReversal.transaction.attributes}" varStatus="statusRow" begin="0" end="2">
      ...
      <c:when test="${tranReversal.transaction.serviceType eq 'PHONE_TOPUP'}">
      ...
      <c:when test="${tranReversal.transaction.serviceType eq 'EXPORT_EPIN'}">
        ...
        <spring:message var="colAttributeOther" code="transaction.api.table.attributeOther"/>
        <c:forEach var="attribute" items="${tranReversal.transaction.attributes}" varStatus="statusRow" begin="0" end="0">
            ...
            <div class="col-sm-9 col-md-10 text-normal">
              <c:choose>
                <c:when test="${attribute.transactionAttributeType eq 'EPIN_PURCHASE_ORDER_CODE'}">
                  <a href="<%=request.getContextPath()%><%=EpinPurchaseOrderController.EPIN_PO_DETAIL%>?poCode=${attribute.transactionAttributeValue}" class="link-active">${attribute.transactionAttributeValue}</a>
                </c:when>
      ...
      <c:when test="${tranReversal.transaction.serviceType eq 'BILL_PAYMENT'}">
      ...
      <c:otherwise>
        ...
        <c:choose>
          <c:when test="${tranReversal.transaction.attributes.size() > 3}">
            <c:forEach var="attribute" items="${tranReversal.transaction.attributes}" varStatus="statusRow" begin="0" end="2">
              <div class="form-group mb-small">
                <label class="col-sm-3 col-md-2 control-label">${(statusRow.index + 1) eq 1 ? colAttribute1 :
                    ((statusRow.index + 1) eq 2 ? colAttribute2 :
          ...
          <c:otherwise>
            <c:forEach var="attribute" items="${tranReversal.transaction.attributes}" varStatus="statusRow">
            ...
            <c:choose>
              <c:when test="${tranReversal.transaction.attributes.size() eq 1}">
                <div class="form-group mb-small">
              ...
              <c:when test="${tranReversal.transaction.attributes.size() eq 2}">
                <div class="form-group mb-small">
                  <label class="col-sm-3 col-md-2 control-label">${colAttribute3}</label>
                  <div class="col-sm-9 col-md-10 text-normal"></div>
                </div>
              ...
              <c:when
                  test="${tranReversal.transaction.attributes.size() eq 3}"><!-- nothing --></c:when>
              <c:otherwise>
                <div class="form-group mb-small">
    ...
    <c:choose>
      <c:when test="${tranReversal.transaction.attributes.size() > 3}">
        <div class="form-group mb-small">
          ...
          <div class="col-sm-9 col-md-10 text-normal">
            <c:forEach var="attribute" items="${tranReversal.transaction.attributes}" varStatus="statusRow" begin="3">
